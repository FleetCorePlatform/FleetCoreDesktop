...
export default function GroupOverviewScreen() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const dronesPromise = apiCall<DroneSummaryModel[]>(
          `/api/v1/groups/${groupUuid}/drones`,
          { limit: 10 },
          'GET'
        );
  ...
  const confirmDelete = async () => {
    ...
    await apiCallFull(`/api/v1/groups/${groupUuid}`, undefined, 'DELETE')
      .then((res) => {
        if (res.status === 304) {
          setDeleteError(
            'Cannot delete group: The group is not empty. Please decommission or move all drones before deleting the group.'
          );
  ...
  const openEditModal = (drone: DroneSummaryModel, field: EditDroneField) => {
    ...
    switch (field) {
      case 'address':
        setEditValue(drone.address);
        break;
      case 'name':
        setEditValue(drone.name);
        break;
      case 'version':
    ...
    <div className="flex flex-col h-full bg-[hsl(var(--bg-primary))] text-[hsl(var(--text-primary))] font-sans overflow-hidden">
      <div className="flex-1 overflow-auto">
        <div className="max-w-[1600px] mx-auto p-4 lg:p-6 space-y-6">
          ...
          <GroupStats drones={drones} firmwareVersion={firmwareVersion} />
