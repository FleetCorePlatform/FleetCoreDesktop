...
export default function DetectionReviewScreen() {
  ...
  const fetchDetections = () => {
    apiCall<Detection[]>(
      '/api/v1/detections',
      { group_uuid: groupUuid || '', mission_uuid: missionUuid || '' },
      'GET'
    )
      .then((res) => {
        setDetections(res);
  ...
  const sendDetectionConfirmation = async (detectionUuid: string, isFalsePositive: boolean) => {
    ...
    apiCallFull(
      '/api/v1/detections',
      { mission_uuid: missionUuid!, detection_uuid: detectionUuid },
      'PATCH',
      payload
    )
      .then((res) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[hsl(var(--bg-primary))] text-[hsl(var(--text-primary))] overflow-hidden font-mono">
      <DetectionHeader navigate={navigate} missionUuid={missionUuid} detections={detections} />
