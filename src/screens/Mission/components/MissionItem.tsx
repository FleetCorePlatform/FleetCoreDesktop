...
export default function MissionItem({ mission, groupUuid, formatTime, getDuration }: {
    ...
    const handleCancelMission = async () => {
        ...
        await apiCallFull(
            ...
            if (res.status === 204) {
                setDetails(prev => prev ? { ...prev, jobStatus: 'CANCELLED', finishedAt: new Date().toISOString() } : null);
                ...
                <AlertDialogContent className="bg-[hsl(var(--bg-secondary))] border-[hsl(var(--border-primary))] text-[hsl(var(--text-primary))]">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-[hsl(var(--text-secondary))]">
                            This action will immediately send a Return-To-Launch (RTL) command to all drones involved in this mission. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
