...
export default function MaintenanceScreen() {
  ...
  const getTypeColor = (type: string) => {
    ...
    if (
      ['hull', 'crash', 'damage', 'critical', 'fail', 'emergency', 'leak'].some((k) =>
        t.includes(k)
      )
    )
      return 'text-red-400 border-red-400/30 bg-red-400/10';
    if (
      ['motor', 'rotor', 'propeller', 'repair', 'replace', 'battery', 'gear', 'servo'].some((k) =>
        t.includes(k)
      )
    )
      return 'text-orange-400 border-orange-400/30 bg-orange-400/10';
    if (
      ['soft', 'firmware', 'patch', 'update', 'glitch', 'boot', 'os', 'ai', 'logic'].some((k) =>
        t.includes(k)
      )
    )
      return 'text-purple-400 border-purple-400/30 bg-purple-400/10';
    if (
      ['sensor', 'lidar', 'calibration', 'cam', 'vision', 'optical', 'radar', 'gps'].some((k) =>
        t.includes(k)
      )
    )
      return 'text-blue-400 border-blue-400/30 bg-blue-400/10';
    if (
      ['routine', 'check', 'inspect', 'clean', 'wash', 'log', 'standard'].some((k) => t.includes(k))
    )
  ...
  const filteredRecords = records.filter((r) => {
    const matchesSearch =
      r.drone_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-[hsl(var(--text-secondary))] font-mono">
