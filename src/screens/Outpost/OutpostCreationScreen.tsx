        ...
        <div
            ...
            <div className="flex flex-1 relative overflow-hidden">
                ...
                `}>
                    ...
                    <div
                        className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-thin scrollbar-thumb-[#3b4354] scrollbar-track-transparent">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-end">
                                ...
                                >
                                    {isLocating ? <Loader2 size={14} className="animate-spin"/> :
                                        <Crosshair size={14}/>}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="space-y-1.5">
                                    <span
                                        className="text-xs font-medium text-[hsl(var(--text-secondary))]">Latitude</span>
                                    <Input
                                        value={coords.lat}
                                ...
                                <div className="space-y-1.5">
                                    <span
                                        className="text-xs font-medium text-[hsl(var(--text-secondary))]">Longitude</span>
                                    <Input
                                        value={coords.lng}
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <h3 className="text-xs font-bold text-[hsl(var(--text-secondary))] uppercase tracking-wider">Geofence
                                    Metrics</h3>
                                {isClosed ? (
                                    <span
                                        className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-medium border border-emerald-500/20">
                                        Boundary Closed
                                    </span>
                                ) : (
                                    <span
                                        className="px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 text-[10px] font-medium border border-yellow-500/20 flex items-center gap-1">
                                        <AlertCircle size={10}/>
                                        Incomplete
                                    </span>
                                )}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div
                                    className="bg-[hsl(var(--bg-tertiary))] border border-[hsl(var(--border-primary))] rounded-lg p-2.5">
                                    <div className="text-[hsl(var(--text-secondary))] text-[10px] mb-0.5">Total Area
                                    </div>
                                    <div className="text-base font-bold text-[hsl(var(--text-primary))]">
                                        {metrics.area > 0 ? metrics.area.toFixed(2) : '-'} <span
                                        className="text-xs font-normal text-[hsl(var(--text-secondary))]">km²</span>
                                    </div>
                                </div>
                                <div
                                    className="bg-[hsl(var(--bg-tertiary))] border border-[hsl(var(--border-primary))] rounded-lg p-2.5">
                                    <div className="text-[hsl(var(--text-secondary))] text-[10px] mb-0.5">Perimeter
                                    </div>
                                    <div className="text-base font-bold text-[hsl(var(--text-primary))]">
                                        {metrics.perimeter > 0 ? metrics.perimeter.toFixed(2) : '-'} <span
                                        className="text-xs font-normal text-[hsl(var(--text-secondary))]">km</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div
                        className="p-5 border-t border-[hsl(var(--border-primary))] bg-[hsl(var(--bg-secondary))] space-y-2">
                        <Button
                            className="w-full h-9 text-sm bg-white text-black hover:bg-gray-200"
                            disabled={!isClosed || !outpostName || isSubmitting || coords.lat === 0 || coords.lng === 0}
                        ...
                        <Link to={"/outposts"}>
                            <Button variant="outline"
                                    className="w-full h-9 text-sm border-[hsl(var(--border-secondary))] hover:bg-[hsl(var(--bg-tertiary))]">Cancel</Button>
                        </Link>
                    </div>
                </aside>
                ...
                <main className="flex-1 relative bg-[hsl(var(--bg-primary))]">
                    ...
                    <div
                        className="absolute top-3.5 left-16 lg:left-4 z-[1000] w-[calc(100%-8rem)] lg:w-[calc(100%-2rem)] max-w-sm transition-[left] duration-200">
                        <div className="relative group">
                            <div
                                className="absolute left-3 top-2.5 text-[hsl(var(--text-secondary))] group-focus-within:text-[hsl(var(--text-primary))] transition-colors">
                                <Search size={18}/>
                            </div>
                            <input
                            ...
                            {showSuggestions && suggestions.length > 0 && (
                                <div
                                    className="absolute top-full left-0 right-0 mt-2 bg-[hsl(var(--bg-tertiary))] border border-[hsl(var(--border-primary))] rounded-lg shadow-xl overflow-hidden max-h-60 overflow-y-auto">
                                    {suggestions.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <span className="truncate">
                                                ...
                                                <span
                                                    className="text-[hsl(var(--text-secondary))] text-xs ml-1.5 opacity-70">
                    ...
                    <AnimatePresence>
                        {locationWarning && (
                            ...
                            >
                                <div
                                    className="flex items-start gap-3 p-4 bg-[#0f1115] border-l-4 border-l-amber-500 border-y border-r border-y-[#282e39] border-r-[#282e39] rounded-r-lg shadow-2xl">
                    ...
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] px-4 max-w-md w-full">
                        <div
                            className="flex items-center gap-2 px-4 py-2 bg-[hsl(var(--bg-tertiary))]/95 backdrop-blur border border-[hsl(var(--border-primary))] rounded-full shadow-xl">
                            <span className="relative flex h-2 w-2 shrink-0">
                                <span
                                    className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isClosed ? 'bg-emerald-400' : 'bg-yellow-400'}`}></span>
                                <span
                                    className={`relative inline-flex rounded-full h-2 w-2 ${isClosed ? 'bg-emerald-500' : 'bg-yellow-500'}`}></span>
                            </span>
                            ...
                            {!isClosed && (
                                <kbd
                                    className="hidden sm:inline-flex h-5 items-center gap-1 rounded border border-[hsl(var(--border-secondary))] bg-[#282e39] px-1.5 font-mono text-[10px] font-medium text-[hsl(var(--text-secondary))] shrink-0">
                                    ESC
                                </kbd>
                            )}
                        </div>
